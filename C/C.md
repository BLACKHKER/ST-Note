...
![image-20231123161401763](https://typora-picture-zhao.oss-cn-beijing.aliyuncs.com/Typora/image-20231123161401763.png)

2. 选择空项目

![image-20231123161607812](https://typora-picture-zhao.oss-cn-beijing.aliyuncs.com/Typora/image-20231123161607812.png)

3. 配置路径信息，创建

![image-20231123161858875](https://typora-picture-zhao.oss-cn-beijing.aliyuncs.com/Typora/image-20231123161858875.png)
...
![image-20231123162629570](https://typora-picture-zhao.oss-cn-beijing.aliyuncs.com/Typora/image-20231123162629570.png)

2. 选择C++文件，<font color="#f40">修改后缀名</font>

> 如果不是这个页面，那么点击显示所有模板即可，默认.cpp(c plus plus C++)注意修改为.c(C)

![image-20231123162804084](https://typora-picture-zhao.oss-cn-beijing.aliyuncs.com/Typora/image-20231123162804084.png)
...
> 流程控制语句，C中没有布尔类型，True/False取决于参数是否为0，非0为真
...
> 注意：else和最近的if匹配，所以代码风格必须加括号，即便只有一行
>
> ```c
> /* 这段代码打印空，因为else匹配的是第二个if(最近的if) */
> #include <stdio.h>
> 
> int main()
> {
...
> 算数运算符运算后自动向上转型，int / float，结果为float
...
![image-20231130140858955](https://typora-picture-zhao.oss-cn-beijing.aliyuncs.com/Typora/image-20231130140858955.png)
...
![image-20231130142308492](https://typora-picture-zhao.oss-cn-beijing.aliyuncs.com/Typora/image-20231130142308492.png)
...
表示静态变量，static修饰的变量不存储在栈区，存储在了静态区，静态区存储变量在**程序结束才销毁**，所以每次访问，数据都是上次修改后的值。
...
如果一个全局变量被static修饰，被修饰全局变量的外部链接属性就变成内部链接属性，那这个全局变量就只能在本源文件中使用，不能在其他源文件内使用。
...
和修饰全局变量类似，限制了函数的使用范围，函数也有外部链接属性，使用了static后，就变成内部链接属性了
...
> 宏的速度比函数的速度快，它<font color="#f40">不是关键字</font>
...
变量在内存中对应的地址，在C语言中可以用一种特殊的变量储存，这个变量就是指针
...
int main()
{
    ...
    /* 修改num的值为88，p_arr数组的第一个元素值为1(0→1) */
    *p_num = 88;
    *p_arr = '1';
...
> 注意：定义指针时，每个指针变量都要有`*`号表示
>
> ```c
> /* 定义多个指针 */
> /* 只有p1是指针类型，p2p3是int类型 */
> int *p1, p2, p3;
> /* 三个指针 */
> int *p1. *p2, *p3;
> ```
...
指针大小取决于操作系统，因为指针是存放地址的，那么地址的大小就决定了指针的大小；

在大多数现代计算机体系结构中，寻址空间的大小是以字节为单位来衡量的。例如，64位操作系统的寻址空间大小为2^64字节，而32位操作系统的寻址空间大小为2^32字节。(参考计算机组成原理)

指针的主要作用是存储变量或数据结构的内存地址，以便能够访问和操作它们。因此，指针的大小必须足够大，以存储寻址空间中的任何地址。

在32位系统中，寻址空间大小为2^32字节，所以指针的大小通常为4字节(32个比特位)。这意味着指针变量可以存储的地址范围是从0到2^32-1。

在64位系统中，寻址空间大小为2^64字节，所以指针的大小通常为8(64个比特位)字节。这意味着指针变量可以存储的地址范围是从0到2^64-1。
...
> 同Java类，一个结构体有多个变量，类似于Java中的属性(类 → 属性)
...
int main()
{
    struct Person xiao_ming = { "小明", 24, "男" };
}
```
...
```c
/* 自己定义一个类似于Java的toString() */
void print(struct Person* p)
{
    printf(" %s\n %d\n %s", (*p).name, (*p).age, (*p).sex);
    
    /* 结构体指针变量 -> 成员名 */
    printf(" %s\n %d\n %s", ps->name, ps->age, ps->sex);
}

int main()
{
    /* 普通打印 */
    printf(" name = %s\n age = %d\n sex = %s\n", xiao_ming.name, xiao_ming.age, xiao_ming.sex);
