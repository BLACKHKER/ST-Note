...
2. 配置附加的包含目录(三方库目录)，目录是静态库[^12]导入的`include`文件夹：
   ...
   ![image-20240307142856563](https://typora-picture-zhao.oss-cn-beijing.aliyuncs.com/Typora/image-20240307142856563.png)
...
动态链接是运行时被链接的，可以选择在程序运行时，装载动态链接库。

在运行时需要把它放在exe文件旁边或某个地方，然后exe可以加载它。这种方式称为动态加载/运行时链接。

因为这种依赖性，需要exe和dll文件在一起。

没有lib，依然可以使用dll，通过函数名访问dll内部的函数。lib文件包含了dll中所有的函数的位置。

静态动态链接主要的区别是库文件是否被编译到exe文件中或链接到exe文件中，或者只是单独的一个文件。
...
const本身改变不了生成的代码(二进制文件)，它本身是一种承诺，你承诺某些东西是不变的，就可以使用它表示

但是你可以打破你的承诺，它是一个你可以绕过的承诺
