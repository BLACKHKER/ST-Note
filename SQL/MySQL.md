...
SELECT后面只能包含:	1.被分组(GROUP BY)的列	2.每个分组只能返回一个值的表达式，如聚合函数
...
> 外键是为了解决数据超出设定的问题：部门表设定有4个部门，员工表的部门ID就不能有5了，解决这个问题就需要外键。外键可以多重继承：A表 -> B表 -> C表。

##### 创建外键

右键创建外键的表 -> 设计表 -> 外键 ：起名一般fk开头fk_，字段是要当外键的字段，参考模式是数据库名；

参考表是要以哪个表为基准，参考字段是要以该表的哪个字段为基准。

最好不用外键，影响性能，阿里巴巴开发规范禁止使用外键。
